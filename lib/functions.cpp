#include "lib.h" //подключаем заголовчный файл из библиотеки 



void checking_data(User&r) //создаем метод 1, проверки данных 
{

	string a, b; //делаем временные перменные для записи
	a = r.get_mail(); //записываем значение 1 поля
	b = r.get_phone_number(); //записываем значение 2 поля
